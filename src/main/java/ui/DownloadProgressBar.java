...
import parameters.download.DownloadProgressHandler;
...
public class DownloadProgressBar implements DownloadProgressHandler
{
    ...
    private static final String FULL = Color.BLUE.getAnsiBC() + " " + Color.WHITE.getAnsiBC();
    ...
    public void showProgressBar(long total, long actual, int length)
    {
        if (total == actual)
        {
            ...
            OUT.print("Download: " + Color.BLUE.getAnsiC() + progressBar + Color.RESET + " DONE   \n\n"); // Jump to next line
        }
        else
        {
            ...
            OUT.print("Download: " + Color.BLUE.getAnsiC() + progressBar + Color.RESET + " " + percentage + "%\r");
        }
    }
}
