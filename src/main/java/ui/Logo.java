...
public class Logo
{
                            ...
                            boolean isCenter, int width)
    {
        ...
        {
            var line = logo[i];
            if (TextUtil.hasNonSpace(line) || (i > 0 && i < logo.length - 1)) // First and last line isn't displayed if empty
            {
                if (isCenter)
                {
                    line = TextUtil.toCenter(line, width);
                }
        ...
        TextUtil.setColor(OUT, colorAfter);
    }
    ...
    private static String[] getLetter(char letter)
    {
        ...
        for (var i = 0; i < result.length; i++)
        {
            ...
            {
                var startIndex = TextUtil.indexOf(LETTER_ROWS[i], "I", letterIndex + 1);
                var endIndex = TextUtil.indexOf(LETTER_ROWS[i], "I", letterIndex + 2);
