...
import parameters.download.DownloadSafetensorsHeader;
import parameters.safetensors.SafetensorsReader;
import network.info.DecoderBlock;
import network.info.DecoderBlockType;
import network.message.toWorker.LoadModelMessage;
import network.info.WorkSegment;
import network.info.WorkSegmentType;
import network.server.state.ModelState;
import network.server.state.WorkerInfo;
...
import static config.BlockType.*;
...
public class LoadModelPlanTask implements Runnable
{
    ...
    public void run()
    {
        var serverState = ServerState.getServerState();
        ...
        {
            ModelState modelState = ServerState.getServerState().addPendingModel(modelId);
            if (modelState != null) // Returns not null if the model was requested the first time, so we have to load it
            {
                try
                {
                    ...
                    var modelConfigPath = ServerState.getServerState().getConfigRoot() + "/" + modelId;
                    var downloadPath = ServerState.getServerState().getDownloadRoot() + "/" + modelId;
